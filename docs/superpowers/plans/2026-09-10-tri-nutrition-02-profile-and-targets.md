...
- **First live run (2026-09-10):** intake worked end to end: it read Garmin first, confirmed weight and body fat, ran the ordered interview, saved the profile and printed the 14-day table. The one prompt miss (asking metric or imperial) is fixed by surfacing `measurementSystem` in `read_garmin_profile`. **Apply failed on the first day** with Garmin `API Error 400`: `macroGoals.carbs / fat / protein must not be null (provided value: null)`. The MCP server sends bare integers under `macroGoals.{carbs,fat,protein}`; Garmin's field is some other type (nested object or percentage). Nothing was written; the 14 days stayed pending as designed. Resolution pending: read `macroGoals` back after goals are set in the Connect app, then fix `to_garmin_call`.
- **Garmin nutrition settings, verified 2026-09-10 by direct probes (spec §15 item 2 is now closed):**
  - `macroGoals` is bare numbers in **grams** (`{"carbs": 275.0, "fat": 92.0, "protein": 206.0}` with `calorieGoal: 2745`). The MCP tool's payload shape was right; the first live failure ("must not be null") happened because the account had no goal type yet (`weightChangeType: NO_GOAL`, `macroGoals: {}`). Choosing "Maintain" in the Connect app created the goal record.
  - Settings are one goal record versioned by `effectiveDate`. Reading any date returns the version in force on that date. **A write for a future date is rejected** (`Effective date 2026-09-11 cannot be later than current date 2026-09-10`; through the MCP tool the same attempt surfaces as `Goal targetDate must be after the asOfDate provided`). A write for today works: PUT returns the full settings object, and a read-back shows the new calorie and macro values (tested 2600/260/80/210, then restored).
  - So Garmin can only ever carry **today's** target. A 14-day horizon cannot be pre-written; each day's target has to be written on that day. This changes spec §2, §6.2 (apply), §9 and the `set_day_targets` semantics; decision pending with Brian (see the plan-3 discussion).
  - The MCP tool's own response reports `carbs_grams/fat_grams/protein_grams` as null even on a successful write; verify writes with a read-back, not the tool's echo.
- **Decision (Brian, 2026-09-10): option 1, Garmin gets today only.** Implemented the same day: `graph/nodes/targets.py::build_horizon` regenerates and upserts the horizon and returns only today's Garmin change; `graph/nodes/apply.py::write_change` is the single Garmin writer and refuses any `set_day_targets` day that is not today; `daily.py` plus `tri-nutrition today [--yes] [--no-live]` is the morning write with a `y/N` gate. The review table still shows the whole horizon. Spec §1, §2, §6.2, §8, §9, §11 and §15 revised. Tests adjusted: one change per run; a future-day change stops the batch with "Garmin can only hold today's target".
- **Root cause of the silent no-op write (2026-09-10, after the first `tri-nutrition today`):** at the pinned garmin_mcp ref `e8554bc`, `set_nutrition_daily_settings` merges the inputs into `activeDailyCalories`, `activeDailyCarbohydrateGrams`, `activeDailyFatGrams`, `activeDailyProteinGrams` and PUTs that. Garmin accepts the request and echoes it, so the tool answers "updated" while `calorieGoal` and `macroGoals` never change (and on an account with `macroGoals: {}` the PUT fails validation, which was the very first live error). The corrected tool (writing `calorieGoal` and `macroGoals.{carbs,fat,protein}`) exists in a later upstream commit. Verified by running the pinned tool in-process with request logging, and by a direct PUT of `calorieGoal` + `macroGoals` through garminconnect 0.3.2, which persists and is what put 2572 / 244 / 147 / 112 on Garmin for 2026-09-10.
- **Pin bump (Brian, 2026-09-10):** `GARMIN_MCP_REF` moved from `e8554bc` to `6c84f7ccf7ab496621357bb4e5e603ee4018fa13` (upstream head; includes the fix). Verified before bumping: a write through a real server at the new ref persists and echoes correctly; tri-core's full live suite (sync reads against Garmin and TrainingPeaks) passes; no code matches on the Garmin error text that changed. Changed in `tri_core.config.Settings`, its test, and `.env.example`; Brian updates his own `.env`.
- **Food-log item shape (verified 2026-09-10 with one logged food):** `mealDetails[].loggedFoods[]` items carry `foodMetaData.foodName`, `servingQty`, `mealId`, `logTimestamp`, and `nutritionContent.{calories, carbs, protein, fat, servingUnit, numberOfUnits}` (grams). The payload also has a top-level `dailyNutritionContent`. Fixture: `tests/fixtures/mcp/get_nutrition_daily_food_log.json`.
