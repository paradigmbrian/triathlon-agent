...
- Correction after the first real conversation: `tp_create_workout`'s MCP schema names the date field `date` (the Python signature's `date_str` is internal). `to_tp_call` sends `date`.
