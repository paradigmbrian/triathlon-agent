...
| K5 | `graph/nodes/planning.py:19-36` and `graph/nodes/nutrition.py:26-57` put only `last_error` in `Proposal.violations` and drop the sub-graph's `pending_violations` (N3, P3); `checkin.py:80-82` approves both gates under `--yes`, so `tri-coach check-in --yes` writes violating fuel notes and designed weeks. | `Proposal.pending_violations` carries the sub-graph's map unchanged, and `Proposal.violations` gains one line per entry (`week of <date>: ...`, `session <id>: ...`, `race note: ...`), so the coach, the terminal review and the web card show them. Under `--yes`, when a gate has flagged changes, `run_checkin` resumes the coach's review with `edit`: every proposal keeps only its clean changes, split with `tri_planning.checkin.changes_without_violations` and `tri_nutrition.repl.split_violating`. It prints each skip and exits 1 when anything was skipped. | `test_nodes.py`: consult and follow-on proposals carry the map and the lines. `test_checkin.py`: a flagged week and a flagged fuel note are left out of both gates' edits, exit 1. |
