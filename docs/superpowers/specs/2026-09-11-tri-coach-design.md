...
| Planning milestone 4 | Rewritten under this spec as *directed* adjust: the adjust sub-agent satisfies a coach brief instead of running its own weekly checklist. | Two judges of what to change would contradict the coach being the sole decider. |
...
- **Directed adjust (milestone 4 rewritten).** The `adjust` node is a `create_agent` sub-agent that receives the coach's brief as its message. Tools: `query_training_db`, a `tp_get_workouts` wrapper, Garmin `get_training_readiness` and `get_hrv_data`, `propose_calendar_changes`, `design_next_week`. Its prompt: satisfy the instruction with the minimal change set; obey `validate.week`, the lever order (swap days, shorten, downgrade intensity, drop, re-plan the week) and ownership (`athlete_requested` only when the brief says the athlete asked); when the instruction is ambiguous, ask one question and stop. `design_next_week` results are merged into the proposal by the node, as plan 4 already decided. `adjust -> review` when changes were proposed (or `END` in embedded mode). The self-directed weekly checklist and the `tri-planning check-in` command are dropped from plan 4; the design-prompt evaluator dataset stays.
...
- Unchanged: `tri-nutrition today` remains the daily Garmin write. `tri-analyze chat`, `tri-planning chat`, `tri-nutrition chat` and `tri-nutrition check-in` stay for debugging.
...
2. **Directed adjust.** Plan 4 rewritten and executed: the adjust sub-agent, `propose_calendar_changes`, `design_next_week`, `tri-planning chat` working in the active phase, the design-prompt evaluator dataset.
