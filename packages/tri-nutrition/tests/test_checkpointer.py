...
async def test_second_process_resumes_and_reads_profile_from_postgres(nocommit, make_deps):
    ...
    garmin = FakeGarmin()
    try:
        async with open_checkpointer(url, STATE_TYPES) as saver, S.open_store(url) as store:
        ...
        async with open_checkpointer(url, STATE_TYPES) as saver2, S.open_store(url) as store2:
        ...
        async with open_checkpointer(url, STATE_TYPES) as saver3, S.open_store(url) as store3:
