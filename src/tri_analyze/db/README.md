...
files. The `db` fixture in `tests/conftest.py` opens a connection, yields it, and rolls back
...
through `tri-analyze sync`, which Brian runs, and through tests inside rolled-back
