...
test("forget returning 404 shows the failure text", async () => {
  const f = vi.spyOn(globalThis, "fetch");
  f.mockImplementation(async (input, init) => {
    const url = String(input);
    if (url === "/api/coach/memory" && (!init || init.method === undefined)) return new Response(JSON.stringify(memory), { status: 200 });
    if (url.startsWith("/api/coach/memory/") && init?.method === "DELETE") return new Response(JSON.stringify({ detail: "not found" }), { status: 404 });
    if (url === "/api/system/status") return new Response(JSON.stringify(status), { status: 200 });
    return new Response("nope", { status: 404 });
  });
  const user = userEvent.setup();
  ...
  await screen.findByText("left knee");
  await user.click(screen.getByRole("button", { name: "forget a1b2c3" }));
  expect(await screen.findByText(/forget failed:/)).toHaveClass("text-danger");
});

test("confirm reset is disabled while the reset request is pending and post is called once on double click", async () => {
  ...
  f.mockImplementation(async (input, init) => {
    ...
    if (url === "/api/coach/reset") {
      await fetchPromise;
      return new Response(JSON.stringify({ running: "sync" }), { status: 409 });
    }
    return new Response("nope", { status: 404 });
  });
  const user = userEvent.setup();
  ...
  expect(f).toHaveBeenCalledWith("/api/coach/reset", expect.objectContaining({ method: "POST" }));
  expect(f.mock.calls.filter((c) => c[0] === "/api/coach/reset")).toHaveLength(1);
});

test("memory fetch 500 shows could not load memory", async () => {
  const f = vi.spyOn(globalThis, "fetch");
  f.mockImplementation(async (input, init) => {
    const url = String(input);
    if (url === "/api/coach/memory" && (!init || init.method === undefined)) return new Response(JSON.stringify({ detail: "server error" }), { status: 500 });
    if (url === "/api/system/status") return new Response(JSON.stringify(status), { status: 200 });
    return new Response("nope", { status: 404 });
  });
  renderWith(<Settings />);
  expect(await screen.findByText(/could not load memory:/)).toHaveClass("text-danger");
});

test("status fetch 500 shows could not load status", async () => {
  const f = vi.spyOn(globalThis, "fetch");
  f.mockImplementation(async (input, init) => {
    const url = String(input);
    if (url === "/api/coach/memory" && (!init || init.method === undefined)) return new Response(JSON.stringify(memory), { status: 200 });
    if (url === "/api/system/status") return new Response(JSON.stringify({ detail: "server error" }), { status: 500 });
    return new Response("nope", { status: 404 });
  });
  renderWith(<Settings />);
  await screen.findByText("left knee");
